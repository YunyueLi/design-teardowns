...
五个工位等距固定，台车按五个停靠位置在 z 方向插值，总行程为 44 个世界单位。`travel = carriageZ - stops[0]` 同时驱动 124 节实例化输送带；节距为 0.62，带节循环回到轨道另一端。滚轮角度按 `travel / 0.23` 更新，反向行程会反转。桌面相机的位置和观察目标同步加上 travel；紧凑布局按 world scale 换算跟随距离。样本和输送带由同一位移推进，没有另一个独立计时动画。
